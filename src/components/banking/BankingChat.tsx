...
function DashboardPanel({ message }: { message: ChatMessage }) {
  if (
    !message.reconciliationResults ||
    !message.reconciliationReviewItems ||
    !message.reconciliationId ||
...
export function BankingChat() {
  ...
  const activeDashboard = [...messages].reverse().find((message) => message.kind === 'dashboard');
  const visibleMessages = activeDashboard
    ? messages.filter((message) => message.kind !== 'dashboard')
    : messages;
  const hasDashboard = !!activeDashboard;

  useEffect(() => {
    window.dispatchEvent(
      new CustomEvent('numera:workspace-mode', { detail: { active: hasDashboard } }),
    );
    return () => {
      window.dispatchEvent(
        new CustomEvent('numera:workspace-mode', { detail: { active: false } }),
      );
    };
  }, [hasDashboard]);
  ...
  return (
    <div
      className={`mx-auto h-[calc(100dvh-7rem)] md:h-[calc(100vh-6.25rem)] min-h-0 ${
        hasDashboard
          ? 'w-full max-w-none md:grid md:grid-cols-[minmax(300px,340px)_minmax(0,1fr)] md:gap-3'
          : 'max-w-3xl'
      }`}
    >
      <div className="flex min-h-0 flex-col">
        <div className={hasDashboard ? 'mb-2' : 'mb-3 md:mb-4'}>
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pb-4 pr-1">
          ...
          {activeDashboard && (
            <div className="md:hidden">
              <DashboardPanel message={activeDashboard} />
            </div>
          )}
          <div ref={bottomRef} />
        </div>
